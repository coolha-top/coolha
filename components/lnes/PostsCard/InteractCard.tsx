...
export default function interactCard({ dataname }) {
  ...
  return (
    <div className='flex justify-items-start md:max-w-[50%] w-full px-6 mt-1 text-base-content/50'>

      {/* 评论 */}
      <div className=" grid justify-start items-center pt-1 rounded-full size-7 w-1/6 hover:text-info" onClick={() => router.push(`/${dataname.by.handle.localName}/posts/${dataname.id}`)} >
        <RiChat1Line className="size-5 " />
        <p className="text-center text-sm">{formatNumberWithUnit(dataname.stats.comments)}</p>
      </div>

      {/* 转发 + 转贴 */}
      <div className=" grid justify-start items-center pt-1 rounded-full size-7 w-1/6  hover:text-success"  >
      ...
      {/* 点赞 */}
      <div className=" grid justify-start items-center pt-1 rounded-full size-7 w-1/6  hover:text-error"  >
        <RiHeart3Line className="size-5 " />{/* RiHeart3Fill  */}
        <p className="text-center text-sm">{formatNumberWithUnit(dataname.stats.upvotes)}</p>
      </div>

      {/* 出版 收藏 */}
      <div className=" grid justify-start items-center pt-1 rounded-full size-7 w-1/6  hover:text-[#00d176]"  >
        <RiTicketLine className="size-5 " />
        <p className="text-center text-sm">{formatNumberWithUnit(dataname.stats.collects)}</p>
      </div>

      {/* 打赏 */}
      {/*       <div className=" grid justify-start items-center pt-1 rounded-full size-7  hover:text-[#d1a000]"  >
      ...
      <div className=" grid justify-start items-center pt-1 rounded-full size-7  w-1/6 hover:text-[#d1a000]"  >
